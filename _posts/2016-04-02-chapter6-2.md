...
#### 6.3.1 프로토타입 체인
- 생성자 함수의 ```prototype``` 객체와 인스턴스 생성 시 내부의 포인터를 이용해 상속을 구현한다. (생성자 함수 ```Parent```(부모), ```Child```(자식)로 표현)
- ```Child```의 ```prototype```을 ```Parent```의 인스턴스로 덮어쓰는 방식이다. 이 때 생성된 연결 고리를 **프로토타입 체인**이라고 한다.
- ```Parent```의 인스턴스가 ```Child```의 ```prototype``` 객체로 할당되면, Child의 ```prototype``` 객체 내부에는 ```Parent```의 ```prototype``` 객체를 가리키는 포인터를 가진다. 이 포인터를 이용해 ```Parent```의 메서드에 접근할 수 있게 된다. 
